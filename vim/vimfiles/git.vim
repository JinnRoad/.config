...
nnoremap \gacp  :!git add . && git commit -v && git push ; git log --graph -25 --pretty=format:'\%Cred\%h\%Creset \%Cgreen\%cs\%Creset \%s \%C(bold blue)\%an\%Creset \%C(yellow)\%D\%Creset' && echo && git status -bsu<cr>
nnoremap \gad   :!git add<cr>
nnoremap \gd    :!git diff<cr>
nnoremap \gl    :!git log --graph -2510 --pretty=format:'\%Cred\%h\%Creset \%Cgreen\%cs\%Creset \%C(bold blue)\%an\%Creset \%s \%C(yellow)\%D\%Creset' && echo && git status -bsu<cr>
